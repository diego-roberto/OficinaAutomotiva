...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void mnLogonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_mnLogonActionPerformed
        //LogonFrame logonFrame = new LogonFrame(funcionario, usuario);
        this.dispose();
    ...
    private void btnOficinaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnOficinaActionPerformed
        OficinaIFrame oficinaFrame = new OficinaIFrame(desktop);
        boolean windowExists = false;

        for (JInternalFrame esseFrame : desktop.getAllFrames()) {
            if (esseFrame.getClass().toString().equalsIgnoreCase(oficinaFrame.getClass().toString())) {
                esseFrame.moveToFront();
                windowExists = true;
            }
        }
        if (!windowExists) {
            desktop.add(oficinaFrame);
            oficinaFrame.setClosable(true);
            oficinaFrame.setIconifiable(true);
            oficinaFrame.setResizable(false);
            oficinaFrame.setVisible(true);
            oficinaFrame.setLocation();
        }
