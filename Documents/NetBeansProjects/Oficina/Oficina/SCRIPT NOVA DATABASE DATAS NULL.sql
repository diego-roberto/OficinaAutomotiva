...
-- -----------------------------------------------------
-- Table `oficina`.`categoria`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `oficina`.`categoria` (
...
CREATE TABLE IF NOT EXISTS `oficina`.`peca` (
  ...
  `quantidade_peca` INT NOT NULL DEFAULT 0,
  PRIMARY KEY (`idpeca`, `categoria_idcategoria`),
  UNIQUE INDEX `idpeca_UNIQUE` (`idpeca` ASC) ,
  INDEX `fk_peca_categoria1_idx` (`categoria_idcategoria` ASC) ,
  CONSTRAINT `fk_peca_categoria1`
    FOREIGN KEY (`categoria_idcategoria`)
    REFERENCES `oficina`.`categoria` (`idcategoria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `oficina`.`ordem` (
  ...
  INDEX `fk_ordem_pagamento1_idx` (`pagamento_idpagamento` ASC) ,
  INDEX `fk_ordem_cliente1_idx` (`cliente_idcliente` ASC) ,
  CONSTRAINT `fk_ordem_pagamento1`
...
CREATE TABLE IF NOT EXISTS `oficina`.`ordem_has_servico` (
  ...
  INDEX `fk_ordem_has_servico_servico1_idx` (`servico_idservico` ASC) ,
  INDEX `fk_ordem_has_servico_ordem1_idx` (`ordem_idordem` ASC) ,
  CONSTRAINT `fk_ordem_has_servico_ordem1`
    FOREIGN KEY (`ordem_idordem`)
    REFERENCES `oficina`.`ordem` (`idordem`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ordem_has_servico_servico1`
...
CREATE TABLE IF NOT EXISTS `oficina`.`ordem_has_peca` (
  ...
  INDEX `fk_ordem_has_peca_peca1_idx` (`peca_idpeca` ASC) ,
  INDEX `fk_ordem_has_peca_ordem1_idx` (`ordem_idordem` ASC) ,
  CONSTRAINT `fk_ordem_has_peca_ordem1`
    FOREIGN KEY (`ordem_idordem`)
    REFERENCES `oficina`.`ordem` (`idordem`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ordem_has_peca_peca1`
    FOREIGN KEY (`peca_idpeca`)
    REFERENCES `oficina`.`peca` (`idpeca`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `oficina`.`ordem_has_funcionario` (
  ...
  INDEX `fk_ordem_has_funcionario_funcionario1_idx` (`funcionario_idfuncionario` ASC) ,
  INDEX `fk_ordem_has_funcionario_ordem1_idx` (`ordem_idordem` ASC, `ordem_pagamento_idpagamento` ASC) ,
  CONSTRAINT `fk_ordem_has_funcionario_ordem1`
    FOREIGN KEY (`ordem_idordem` , `ordem_pagamento_idpagamento`)
    REFERENCES `oficina`.`ordem` (`idordem` , `pagamento_idpagamento`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ordem_has_funcionario_funcionario1`
    FOREIGN KEY (`funcionario_idfuncionario`)
    REFERENCES `oficina`.`funcionario` (`idfuncionario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
